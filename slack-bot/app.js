...
app.locals.slack_channel = config.slack.channel;
app.locals.slack_webhook = config.slack.webhook;
 
slack = new Slack();
slack.setWebhook(app.locals.slack_webhook);
...
app.post('/', function(req, res) {
  ...
  // Pokemon ID
  pokemon_id = req.body.message.pokemon_id;
  // Time the Pokemon Expires and disappears from map
  disappear_time = req.body.message.disappear_time;
  // Pokemon Longitude
  longitude = req.body.message.longitude;
  // Pokemon Latitude
  latitude = req.body.message.latitude;

  // Pokemon IDs that you want to ignore
  if (
       pokemon_id == 10 // Caterpie
    || pokemon_id == 13 // Weedle
    || pokemon_id == 14 // Kakuna
    || pokemon_id == 16 // Pidgey
    || pokemon_id == 17 // Pidgeotto
    || pokemon_id == 19 // Rattata
    || pokemon_id == 20 // Raticate
    || pokemon_id == 21 // Spearow
    || pokemon_id == 29 // Nidoran (female)
    || pokemon_id == 32 // Nidoran (male)
    || pokemon_id == 41 // Zubat
    || pokemon_id == 43 // Oddish
    || pokemon_id == 46 // Paras
    || pokemon_id == 48 // Venonat
    || pokemon_id == 69 // Bellsprout
    || pokemon_id == 90 // Shellder
  ) {
    return
  }

  // Default timezone is UTC in the webhook, subtract 4 hours for EST
  a = moment.unix(disappear_time).subtract(4, 'hours');
  expires = a.format("MM.DD HH:mm:ss");

  /**********************************************
   * Message sent to Slack
   * Format:
   * Pokemon Name - [data time expires] - link to google maps
  */
  
  message = pokemonIdtoName(pokemon_id) + " - [" + expires + "] <https://www.google.com/maps/dir/Current+Location/" + latitude + "," + longitude + "|(" + latitude + ", " + longitude + ")>";

  slack.webhook({
    channel: app.locals.slack_channel,
    text: message
